...
class NodeTree
  ...
  def initialize(list_nodes)
    unless list_nodes.empty?
      @root_id = list_nodes[0].id
    end
    @nodes = {}
    @recs = {}
  ...
  def n_child_artists(node_id)
    ...
    self.list_node(node_id).listable_type == 'Album' ? children.map{|nid| self.list_node(nid).listable.artist_id}.uniq.size : 0
  end
  def album_struct(st,album)
    ...
    artist_ids = children.map{|nid| self.list_node(nid).listable.artist_id}.uniq
  ...
  def create_playdata(node)
    if node && node.listable_type == 'AudioFile'
      "list_node-#{node.id}--audio_file-#{node.listable.id}--album-#{node.listable.album_id}" 
    end
  end
  def playdata_in_tree(playdata)
    ln_el,af_el,alb_el = playdata.split('--');
    str,ln_id = ln_el.split('-')
    @nodes[ln_id.to_i]
  end
  def audio_file_node_ids 
    @audio_file_node_ids ||= self.list_node_ids.select{|nid| list_node(nid).listable_type == 'AudioFile'}
  end
  def audio_file_list_nodes 
    @audio_file_list_nodes ||= self.audio_file_node_ids.map{|nid| list_node(nid)}
  end
  def first_audio_file_node
    self.audio_file_list_nodes.empty? ? nil : self.audio_file_list_nodes.first
  end
  def first_audio_file_node_id
    self.audio_file_list_nodes.empty? ? nil : self.audio_file_list_nodes.first.id
  end
  def next_audio_file_node_id(node_id)
    idx = self.audio_file_list_nodes.index{|ln| ln.id == node_id}
    next_node =  self.audio_file_list_nodes[idx+1]
    next_node ? next_node.id : nil
  end
  def album_node_ids 
    @album_node_ids ||= self.list_node_ids.select{|nid| list_node(nid).listable_type == 'Album'}
  end
  def audio_file_recs
    @recs[:audio_files] ||= Hash[ self.audio_file_node_ids.map{|nid| [list_node(nid).listable.id,list_node(nid).listable]} ]
  end
  def audio_file_ids
  ...
  def audio_file_album_ids
    audio_files.map{|af| af.album_id}
  end
  def album_recs
    @recs[:albums] ||= self.create_album_recs
  end
  def artist_recs
    @recs[:artists] ||= self.create_artist_recs
  end
  def song_recs
    @recs[:songs] ||= self.create_song_recs
  end
  def file_dir_recs
    @recs[:file_dirs] ||= self.create_file_dir_recs
  end

  def create_song_recs
    af_song_ids = self.audio_files.map{|af| af.song_id}.uniq
    af_songs = Song.find(af_song_ids)
    Hash[af_songs.map{|s| [s.id,s]}]
  end
  def create_file_dir_recs
    af_file_dir_ids = self.audio_files.map{|af| af.file_dir_id}.uniq
    af_file_dirs = FileDir.find(af_file_dir_ids)
    Hash[af_file_dirs.map{|s| [s.id,s]}]
  end
  def create_artist_recs
    af_artist_ids = self.audio_files.map{|af| af.artist_id}.uniq
    af_artists = Artist.find(af_artist_ids)
    Hash[af_artists.map{|s| [s.id,s]}]
  end
  def create_album_recs
    album_recs = Hash[ self.album_node_ids.map{|nid| [list_node(nid).listable.id,list_node(nid).listable]} ]
    af_album_ids = self.audio_file_album_ids
